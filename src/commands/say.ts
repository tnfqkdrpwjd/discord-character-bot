...
export const sayCommand = new SlashCommandBuilder()
  ...
  .addStringOption((opt) =>
    opt.setName("캐릭터").setDescription("말할 캐릭터").setRequired(true).setAutocomplete(true)
  )
  .addStringOption((opt) => opt.setName("내용").setDescription("대사 내용").setRequired(true));

export async function handleSayAutocomplete(interaction: AutocompleteInteraction) {
...
export async function handleSayCommand(interaction: ChatInputCommandInteraction) {
